...
function storageFromCard(card){const p=parts(card);return p.find(x=>!['tablet','wearable'].includes(key(x))&&key(x)!==key(brandFromCard(card)))||''}
...
function groupedCard(cards){
 ...
 function sync(){
  $$('.morley-variant-chip',storageRow).forEach(b=>{const active=b.dataset.choiceValue===selected.storage;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));const possible=variants.some(v=>v.storage===b.dataset.choiceValue&&(!selected.connectivity||v.connectivity===selected.connectivity));b.disabled=!possible&&variants.some(v=>v.storage===b.dataset.choiceValue)===false});
  $$('.morley-variant-chip',connectivityRow).forEach(b=>{const active=b.dataset.choiceValue===selected.connectivity;b.classList.toggle('active',active);b.setAttribute('aria-pressed',String(active));const possible=variants.some(v=>v.connectivity===b.dataset.choiceValue&&(!selected.storage||v.storage===selected.storage));b.disabled=!possible&&variants.some(v=>v.connectivity===b.dataset.choiceValue)===false});
