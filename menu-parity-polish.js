(()=>{
  ...
  function enhanceAbout(d){
    ...
    box.innerHTML=`<b style="color:#fff">Diagnostics snapshot</b><br><span style="color:#9db0c9">Network:</span> ${navigator.onLine?'Online':'Offline'}<br><span style="color:#9db0c9">Secure session:</span> ${sessionState()}<br><span style="color:#9db0c9">Local workspace:</span> ${s.count} keys · ~${Math.max(1,Math.round(s.bytes/1024))} KB<br><span style="color:#9db0c9">Browser:</span> ${navigator.userAgent.replace(/</g,'&lt;').slice(0,150)}`;
