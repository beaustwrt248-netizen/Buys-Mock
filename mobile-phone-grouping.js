...
const storageFromPart=part=>{const text=norm(part);if(!text)return'';const after=text.match(/\b(?:internal\s+storage|device\s+storage|storage|capacity|rom)\s*[:\-=]?\s*(\d+(?:\.\d+)?)\s*(TB|GB|MB)\b/i);if(after)return capacity(after[1],after[2]);const before=text.match(/\b(\d+(?:\.\d+)?)\s*(TB|GB|MB)\s*(?:internal\s+storage|device\s+storage|storage|capacity|rom)\b/i);if(before)return capacity(before[1],before[2]);if(/\b(?:ram|memory\s+ram|expandable|micro\s*sd|microsd|sd\s*card)\b/i.test(text))return'';const bare=text.match(/^\s*(\d+(?:\.\d+)?)\s*(TB|GB|MB)\s*$/i);return bare?capacity(bare[1],bare[2]):''};
