...
require_markers("android/app/src/main/java/com/buysloans/hub/DashboardActivity.kt", "CATALOGUE(\"Catalogue\", MorleyIcons.Categories)", "SCAN(\"Scan\", MorleyIcons.Phone)", "TRADE(\"Trade\", MorleyIcons.Money)", "General Buys / GP", "CategoriesPricingScreen", "ConsolePricingScreen")
forbid_markers("android/app/src/main/java/com/buysloans/hub/DashboardActivity.kt", "NavCard(MorleyIcons.Categories", "NavCard(MorleyIcons.Console", "ExpandedDestination.CATEGORIES", "ExpandedDestination.GP", "ExpandedDestination.MORE")
...
# Admin web must mirror native Admin authority, not legacy browser superset authority.
...
require_markers("admin/workspace.html", "workspace-template.html?v=2", "admin-native-parity-core.js?v=1", "catalogue-readonly-parity.js?v=1", "admin-app-parity.js?v=4", "adminWorkspace='ready'")
...
forbid_markers("admin/admin-user-access-parity.js", 'data-user-action="force_signout"', 'data-user-action="delete"')
require_markers("admin/catalogue-readonly-parity.js", "device_catalog")
require_markers("admin/targeted-notifications.js", "audience:all", "user:", "notification_job", "send-admin-notification", "send-morley-email")
forbid_markers("admin/targeted-notifications.js", "device:", "target_installation_id")
...
for path in ["admin/styles.css","admin/support-tickets.js","admin/audit-triage.js","admin/user-management-policy.js","admin/invites.js","admin/download-invites.js","admin/targeted-notifications.js","admin/admin-native-parity-core.js","admin/catalogue-readonly-parity.js","admin/admin-user-access-parity.js"]: require_file(path)
# Dormant legacy files remain checked in for compatibility/reference but are not part of the parity loader.
for path in ["admin/app.js","admin/control-governance.js","admin/pricing-management.js","admin/release-control.js"]: require_file(path)
