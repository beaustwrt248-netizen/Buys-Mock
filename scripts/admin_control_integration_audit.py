...
require("callback:function(token)" in login_security, "Admin web Turnstile token callback is missing")
...
require("script.onerror" in login_security, "Admin web Turnstile script failure recovery is missing")
...
require("event.source===frame.contentWindow" in login_security, "Admin web stale bridge source rejection is missing")
require("event.origin===window.location.origin" in login_security, "Admin web stale bridge origin rejection is missing")
require("payload.type==='token'" not in login_security, "Admin web must not accept browser CAPTCHA tokens through the retired iframe bridge")
require("window.parent.postMessage" in turnstile_html and "source:'morley-turnstile'" in turnstile_html, "Native/WebView Turnstile page does not retain its scoped delivery bridge")
...
print("Verified direct browser Admin auth with mobile recovery, native Android session ownership/workspaces, privileged API contracts and OTA governance.")
