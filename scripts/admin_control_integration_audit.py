...
require("adminTurnstileWidget" in login_security, "Admin web direct Turnstile widget is missing")
require("legacyFrame.replaceWith(widget)" in login_security, "Admin web does not replace the legacy iframe placeholder")
require("challenges.cloudflare.com/turnstile/v0/api.js?render=explicit" in login_security, "Admin web does not load explicit Turnstile at top level")
require("window.turnstile.render" in login_security, "Admin web does not render the top-level Turnstile challenge")
require("callback:function(token)" in login_security, "Admin web direct Turnstile token callback is missing")
require("function startFallback" in login_security, "Admin web compatibility Turnstile fallback is missing")
require("turnstile.html?v=7&browser=1" in login_security, "Admin web compatibility challenge URL is missing")
require("mode='fallback'" in login_security, "Admin web does not enter compatibility challenge mode")
...
require("script.onerror" in login_security, "Admin web direct Turnstile script failure recovery is missing")
require("visibilitychange" in login_security and "pageshow" in login_security, "Admin web mobile lifecycle recovery is incomplete")
require("event.source===frame.contentWindow" in login_security, "Admin web compatibility bridge source validation is missing")
require("event.origin===window.location.origin" in login_security, "Admin web compatibility bridge origin validation is missing")
require("payload.source!=='morley-turnstile'" in login_security, "Admin web compatibility bridge identity validation is missing")
require("payload.type==='token'" in login_security, "Admin web compatibility token bridge is missing")
...
print("Verified dual-path browser Admin auth with mobile recovery, native Android session ownership/workspaces, privileged API contracts and OTA governance.")
