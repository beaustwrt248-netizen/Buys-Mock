...
create table if not exists public.nova_knowledge_items (
  ...
  search_vector tsvector generated always as (to_tsvector('english'::regconfig,coalesce(title,'') || ' ' || coalesce(content,'') || ' ' || coalesce(source_label,'') || ' ' || coalesce(source_filename,'') || ' ' || coalesce(version_label,''))) stored
);
