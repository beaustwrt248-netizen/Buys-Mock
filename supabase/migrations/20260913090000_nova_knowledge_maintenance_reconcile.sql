-- Reconcile Nova knowledge maintenance hardening on current production baseline.
-- Additive only. Recurring cron intentionally remains disabled while issue #1872 is unresolved.
...
comment on function public.nova_claim_embedding_chunks(integer) is
  'Service-role-only bounded Nova embedding claim using SKIP LOCKED. Does not change knowledge content.';
...
as $function$
  select jsonb_build_object(
    ...
    'latest_maintenance_status', (select status from public.nova_knowledge_maintenance_runs order by started_at desc limit 1),
...
-- IMPORTANT: do not schedule or invoke maintenance here.
-- Recurring maintenance remains disabled until issue #1872's scheduler authorization path is verified stable.