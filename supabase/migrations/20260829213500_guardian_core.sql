...
create index if not exists guardian_incidents_state_created_idx on public.guardian_incidents(state, created_at desc);
create index if not exists guardian_incidents_risk_created_idx on public.guardian_incidents(risk_level, created_at desc);
...
create policy guardian_settings_admin_read on public.guardian_settings for select to authenticated using (private.is_admin_or_manager());
drop policy if exists guardian_settings_admin_update on public.guardian_settings;
create policy guardian_settings_admin_update on public.guardian_settings for update to authenticated using (private.is_admin_or_manager()) with check (private.is_admin_or_manager());
drop policy if exists guardian_incidents_admin_read on public.guardian_incidents;
create policy guardian_incidents_admin_read on public.guardian_incidents for select to authenticated using (private.is_admin_or_manager());
drop policy if exists guardian_incidents_admin_update on public.guardian_incidents;
create policy guardian_incidents_admin_update on public.guardian_incidents for update to authenticated using (private.is_admin_or_manager()) with check (private.is_admin_or_manager());
...
create trigger trg_guardian_queue_support_ticket after insert on public.support_tickets for each row execute function private.guardian_queue_support_ticket();
...
create trigger trg_guardian_touch_settings before update on public.guardian_settings for each row execute function private.guardian_touch_row();
drop trigger if exists trg_guardian_touch_incident on public.guardian_incidents;
create trigger trg_guardian_touch_incident before update on public.guardian_incidents for each row execute function private.guardian_touch_row();
...
begin
  if tg_table_name = 'guardian_settings' then
    ...
    values(auth.uid(), 'guardian_settings_updated', 'guardian_settings', 'singleton', jsonb_build_object('enabled',new.enabled,'auto_fix_enabled',new.auto_fix_enabled,'max_auto_risk',new.max_auto_risk,'require_human_for_code',new.require_human_for_code));
  else
    insert into public.admin_audit_log(actor_user_id, action, target_type, target_id, details)
    values(auth.uid(), 'guardian_incident_updated', 'guardian_incident', new.id::text, jsonb_build_object('state',new.state,'risk_level',new.risk_level,'requires_approval',new.requires_approval));
  end if;
  return new;
end;
$$;
...
create trigger trg_guardian_audit_settings after update on public.guardian_settings for each row execute function private.guardian_audit_change();
drop trigger if exists trg_guardian_audit_incident on public.guardian_incidents;
create trigger trg_guardian_audit_incident after update on public.guardian_incidents for each row execute function private.guardian_audit_change();
