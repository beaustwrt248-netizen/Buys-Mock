-- Admin Control 2.0 safe remote configuration.
-- Narrow feature_flags writes to maintenance controls only while preserving all
-- existing pricing/scanner/history flags byte-for-byte. Release configuration
-- validation and audit behavior remain unchanged.
...
begin
  ...
  if config_key = 'feature_flags' then
    ...
    if (config_value - 'maintenanceMode' - 'maintenanceMessage')
       <> (existing_flags - 'maintenanceMode' - 'maintenanceMessage') then
      raise exception 'only maintenanceMode and maintenanceMessage may be changed remotely';
    end if;
  ...
  values(auth.uid(),'config_updated','config',config_key,
    case when config_key='feature_flags'
      then jsonb_build_object('maintenanceMode',config_value->'maintenanceMode','maintenanceMessage',config_value->'maintenanceMessage')
...
comment on function private.admin_set_config_impl(text,jsonb) is
  'Privileged Admin/Manager config implementation. feature_flags changes are limited to maintenanceMode/message; all changes are audited.';
