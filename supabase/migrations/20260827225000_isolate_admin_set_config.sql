...
begin
  ...
  if jsonb_typeof(config_value) <> 'object' then
    raise exception 'config value must be a JSON object';
  end if;

  if config_key = 'current_release' then
    begin
      release_code := (config_value->>'versionCode')::integer;
    exception when others then
      raise exception 'current release versionCode must be a positive integer';
    end;
    ...
    if release_code < 1 or release_name = '' then
      raise exception 'current release identity is invalid';
    end if;
    if apk_url not like 'https://github.com/beaustwrt248-netizen/Buys-Mock/releases/download/%' then
      raise exception 'current release APK URL is not trusted';
    end if;
    if sha256 !~ '^[0-9a-f]{64}$' then
      raise exception 'current release SHA-256 is invalid';
    end if;

    select value into existing_release
    ...
    if existing_release is not null then
      ...
      if existing_code is not null and release_code < existing_code then
        raise exception 'current release rollback is not allowed';
      end if;
      if existing_code = release_code and (
        ...
        raise exception 'release identity cannot change for an existing versionCode';
      end if;
    end if;
    ...
    begin
      minimum_code := (config_value->>'versionCode')::integer;
    exception when others then
      raise exception 'minimum versionCode must be a positive integer';
    end;
    minimum_name := btrim(coalesce(config_value->>'versionName',''));
    if minimum_code < 1 or minimum_name = '' then
      raise exception 'minimum supported version identity is invalid';
    end if;
    select nullif(value->>'versionCode','')::integer into release_code
    from public.app_config
    where key='current_release';
    if release_code is null then
      raise exception 'current release is not configured';
    end if;
    if minimum_code > release_code then
      raise exception 'minimum supported version cannot exceed current release';
    end if;
    if config_value ? 'forceUpdate' and jsonb_typeof(config_value->'forceUpdate') <> 'boolean' then
      raise exception 'forceUpdate must be boolean';
    end if;
  end if;
