...
function validEmail(value: string) {
  return value.length >= 3 && value.length <= 254 && value.includes('@') && value.split('@')[1]?.includes('.');
}
...
async function revokeUserSessions(targetUserId: string) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (token.split('.').length !== 3) return reply({ error: 'Invalid session' }, 401);
    ...
    if (action === 'create_user') {
      ...
      const { data: created, error: createError } = await admin.auth.admin.createUser({
        ...
        user_metadata: {
          ...
          temporary_password_account: true,
      ...
      if (profileError) {
        await admin.auth.admin.deleteUser(targetUser);
        throw profileError;
      }

      await admin.from('admin_audit_log').insert({
        actor_user_id: user.id,
        action: 'user_create_temporary_password',
        target_type: 'user',
        target_id: targetUser,
        details: { email, role, skip_email_verification: true, must_change_password: true },
      });
    ...
    if (targetUser === user.id && ['disable', 'force_signout', 'delete', 'set_role', 'reset_password'].includes(action)) {
      return reply({ error: 'You cannot perform this action on your own active admin account' }, 400);
    }
    ...
    if (action === 'force_signout') {
      const revokedSessions = await revokeUserSessions(targetUser);
      details = { ...details, revoked_sessions: revokedSessions };
      ...
      if (!['staff', 'manager', 'admin'].includes(role)) return reply({ error: 'Invalid role' }, 400);
      const { error } = await admin
        .from('profiles')
        .update({ role, updated_at: new Date().toISOString() })
        .eq('id', targetUser);
      if (error) throw error;
      details = { ...details, new_role: role };
      ...
      const { error } = await admin
        ...
        .eq('id', targetUser);
      if (error) throw error;
      const { error: authError } = await admin.auth.admin.updateUserById(targetUser, {
        user_metadata: { first_name: firstName, last_name: lastName, full_name: displayName },
      });
      ...
      const { error: updateError } = await admin.auth.admin.updateUserById(targetUser, {
        password: temporaryPassword,
      ...
      const { error } = await admin.auth.admin.deleteUser(targetUser, false);
      if (error) throw error;
    } else {
