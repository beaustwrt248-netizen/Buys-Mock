...
function extractSources(data:any){
  ...
  for(const item of data?.output||[]){
    ...
    for(const source of sources){const u=String(source?.url||"");if(/^https?:\/\//i.test(u)&&!urls.includes(u))urls.push(u)}
  }
  return urls.slice(0,8);
}
async function aiDiagnosis(input:any,repo:any[],enabled:boolean,model:string,allowWebResearch:boolean){
  if(!enabled||!OPENAI_API_KEY)return null;
  const prompt=`You are Morley Guardian, a cautious software incident diagnosis agent. Diagnose only; never claim to have changed code, merged, deployed, changed auth/RLS, accessed secrets, or altered production data. ${allowWebResearch?"You may use web search when current official documentation or external technical evidence would materially improve the diagnosis. Prefer primary/official documentation and do not use web search for secrets, private data, credentials or user-identifying information.":"External web research is disabled for this incident."} Return JSON only with keys: diagnosis_summary (string <=1800), proposed_action (string <=3500), reproduction_summary (string <=1800), test_plan (string <=1800), confidence (0..1), risk_level (low|medium|high|critical), requires_approval (boolean), likely_files (array of strings max 6), needs_external_research (boolean).\n\nINCIDENT:\n${JSON.stringify(input).slice(0,16000)}\n\nREAD-ONLY REPOSITORY CONTEXT:\n${JSON.stringify(repo).slice(0,50000)}`;
  const request:any={model,input:prompt,reasoning:{effort:"medium"},max_output_tokens:1800};
  if(allowWebResearch){request.tools=[{type:"web_search_preview",search_context_size:"medium"}];request.include=["web_search_call.action.sources"]}
  const r=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify(request)});
  if(!r.ok)throw new Error(`AI response ${r.status}`);const data=await r.json();const text=extractResponseText(data).trim();const start=text.indexOf("{"),end=text.lastIndexOf("}");if(start<0||end<=start)throw new Error("AI returned no JSON diagnosis");const x=JSON.parse(text.slice(start,end+1));return{state:"proposed",risk_level:["low","medium","high","critical"].includes(x.risk_level)?x.risk_level:"medium",confidence:Math.max(0,Math.min(1,Number(x.confidence)||.5)),diagnosis_summary:clip(x.diagnosis_summary,1800),proposed_action:clip(x.proposed_action,3500),reproduction_summary:clip(x.reproduction_summary,1800),test_plan:clip(x.test_plan,1800),auto_fix_eligible:false,requires_approval:x.requires_approval!==false,last_error_code:null,likely_files:Array.isArray(x.likely_files)?x.likely_files.slice(0,6):[],needs_external_research:!!x.needs_external_research,research_sources:extractSources(data)};
}
...
Deno.serve(async(req:Request)=>{
  ...
  if(settings.external_research_enabled)await activity(incidentId,"researching","working","Controlled documentation research is available.","Guardian may use server-side web search when current external technical evidence is needed. Search results cannot authorize code or deployment actions.",48,visibility);
  let triage:any=null;try{await activity(incidentId,"diagnosing","working","AI-assisted diagnosis is running.",settings.ai_enabled!==false?`Model: ${settings.agent_model||DEFAULT_MODEL}`:"AI assistance is disabled; Guardian will use deterministic triage.",50,visibility);triage=await aiDiagnosis(input,repo,settings.ai_enabled!==false,String(settings.agent_model||DEFAULT_MODEL),!!settings.external_research_enabled)}catch(e){await activity(incidentId,"diagnosing","warning","AI diagnosis was unavailable; Guardian continued safely.",clip(e instanceof Error?e.message:e,500),52,"admin");}
  triage=triage||fallback(input);
  if(triage.research_sources?.length)await activity(incidentId,"researching","success","Guardian used external technical sources.",triage.research_sources.join(" • "),58,"admin");
  else if(triage.needs_external_research&&!settings.external_research_enabled)await activity(incidentId,"researching","warning","Guardian identified a research need, but external research is disabled.","Enable external documentation research in Guardian intelligence controls if you want Guardian to expand this investigation.",58,visibility);
  ...
  return json({ok:true,incident_id:incidentId,state:triage.state,risk_level:triage.risk_level,confidence:triage.confidence,repository_files:repo.map(x=>x.path),research_sources:triage.research_sources||[],ai_used:!!OPENAI_API_KEY&&settings.ai_enabled!==false});
});
