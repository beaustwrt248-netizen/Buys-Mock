...
Deno.serve(async (req: Request) => {
  ...
  const dispatchToken = String(body?.dispatch_token ?? "").trim();
  if (!uuid.test(incidentId) || !uuid.test(dispatchToken)) return json({error:"invalid_dispatch"},400);
  ...
  const {data:incident,error:incidentError}=await sb.from("guardian_incidents").select("id,ticket_id,state,attempt_count,dispatch_token").eq("id",incidentId).single();
  if(incidentError||!incident) return json({error:"dispatch_rejected"},401);
  if(String(incident.dispatch_token)!==dispatchToken) return json({error:"dispatch_rejected"},401);
  if(incident.state!=="queued") return json({ok:true,skipped:`state_${incident.state}`});

  const {data:claimed,error:claimError}=await sb.from("guardian_incidents").update({state:"diagnosing",attempt_count:Number(incident.attempt_count??0)+1,last_worker_at:new Date().toISOString(),worker_version:"guardian-live-v2",last_error_code:null,dispatch_token:crypto.randomUUID()}).eq("id",incidentId).eq("state","queued").eq("dispatch_token",dispatchToken).select("id").maybeSingle();
  ...
  const {error:updateError}=await sb.from("guardian_incidents").update({...triage,classification:String(ticket.category??"other"),worker_version:"guardian-live-v2",last_worker_at:new Date().toISOString()}).eq("id",incidentId);
