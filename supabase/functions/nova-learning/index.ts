...
async function harvest(userId:string){
  const [incidentsRes,repairsRes,valsRes]=await Promise.all([
  ...
  for(const i of incidents){
    ...
    const verified=!!i.verified_at||state==='resolved';
  ...
  for(const r of repairs){
    ...
    const success=['completed','verified','merged','applied'].includes(status)||!!r.completed_at;
    const failure=['failed','rejected','quarantined'].includes(status);
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(action==='feedback'){
      const domain=clean(body?.domain,40),summaryText=clean(body?.summary,1200);
