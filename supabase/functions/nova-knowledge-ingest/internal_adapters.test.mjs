...
test('device adapter preserves AU evidence without source row identifiers', () => {
  const doc = adaptDeviceCatalogRow({
    ...
    key_specs: { display: '6.7 inch' }, aliases: ['Edge Seventy'], search_text: 'do-not-copy',
  });
  ...
  assert.doesNotMatch(JSON.stringify(doc), /987|do-not-copy/);
  assert.equal(doc.source_uri, 'https://example.test/device');
});

test('support adapter excludes customer text, IDs and raw diagnostics', () => {
  const doc = adaptSupportTicketRow({
    id: 'ticket-secret-id', user_id: 'user-secret-id', assigned_to: 'agent-secret-id',
    category: 'login', subject: 'person@example.com', description: 'reset token ABC123',
    status: 'resolved', priority: 'high', app_version: '2.15.101', app_version_code: 215101,
    device_model: 'SM-S948B', android_version: '16', diagnostics: { access_token: 'secret', crash: 'raw dump' },
...
test('Guardian incident preserves approval boundary while stripping actor and dispatch identifiers', () => {
  const doc = adaptGuardianIncidentRow({
    ...
    github_branch: 'guardian/fix-recent', github_pr_number: 123, last_error_code: 'ReferenceError',
    reproduction_summary: 'Open dashboard', test_plan: 'Run runtime regression', resolution_summary: 'Source repaired',
    occurrence_count: 4, last_seen_at: '2026-09-12T00:00:00Z', app_version: '2.15.101', route: '/dashboard', diagnostic_message: 'recent is not defined',
    diagnostic_metadata: { token: 'bad' }, updated_at: '2026-09-12T00:00:00Z', verified_at: '2026-09-12T00:00:00Z',
  });
  const serialized = JSON.stringify(doc);
...
test('verified Guardian learning strips evidence secrets and creator identifiers', () => {
...
test('operational adapter emits no document for an empty authoritative dataset', () => {
