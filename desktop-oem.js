...
function clearStats(reason='No reliable whole-device comparables found.'){if($('deskNew'))$('deskNew').textContent=money(0);if($('deskUsed'))$('deskUsed').textContent=money(0);if($('deskMax'))$('deskMax').textContent=money(0);if($('deskAverage'))$('deskAverage').textContent=money(0);if($('deskVerdict')){$('deskVerdict').textContent='HOLD';$('deskVerdict').className='warn'}if($('deskStatus'))$('deskStatus').textContent=`HOLD • ${reason}`;if($('deskAdd'))$('deskAdd').disabled=true;if($('deskSave'))$('deskSave').disabled=true;if($('deskSaveStock'))$('deskSaveStock').disabled=true;oemState={used:0,newP:0,max:0,verdict:'HOLD'};try{lastDesktop=null}catch{}}
...
async function analyseOEM(o){const q=queryFor(o),btn=$('deskAnalyse');if(btn)btn.disabled=true;showMode(o);if($('deskStatus'))$('deskStatus').textContent=`Searching exact whole-system market: ${q}`;try{const d=await market(q,40,'device'),np=d.google?.pricing?.competitiveLow||d.google?.pricing?.typicalNew||0,ep=d.ebay?.pricing||{},used=ep.typicalUsed||0,gc=d.google?.analysedListings||0,ec=d.ebay?.analysedListings||0,noReliable=!!ep.noReliableComparables||(!used&&ec===0);if(noReliable){clearStats(`No reliable exact used comparables for ${o.model}. ${gc} Google new offers • ${ec} eBay used matches.`);return}const {max,v}=setStats(np,used);oemState={o,q,newP:np,used,max,verdict:v,data:d};if($('deskStatus'))$('deskStatus').textContent=`OEM whole-device valuation • ${gc} Google new offers • ${ec} eBay used matches`;try{lastDesktop={name:o.model,cost:Number($('deskAsk')?.value)||Math.round(max),sell:Math.round(used),category:'Desktop PC',notes:q}}catch{}if($('deskAdd'))$('deskAdd').disabled=false;if($('deskSave'))$('deskSave').disabled=false;if($('deskSaveStock'))$('deskSaveStock').disabled=false;if(typeof activity==='function')activity(`${o.model} valuation • ${v}`,money(used));if(typeof save==='function')save()}catch(e){clearStats(String(e?.message||e))}finally{if(btn)btn.disabled=false}}
function boot(){const specs=$('deskSpecs'),detect=$('deskDetect'),analyse=$('deskAnalyse');if(!specs||!detect||!analyse)return;detect.onclick=()=>{const o=detectOEM(specs.value);oemState=o?{o}:null;if(o){clearStats('OEM system detected. Run Detect + Price All for exact whole-device pricing.');showMode(o);const first=document.querySelector('#parts .pq');if(first){first.value=queryFor(o);const title=first.closest('.part')?.querySelector('b');if(title)title.textContent='Whole OEM System'}}else if(typeof parseDesk==='function'){document.querySelectorAll('#parts .part b').forEach((b,i)=>{const names=['CPU','GPU','Motherboard','RAM','SSD','Power Supply','Case','CPU Cooler'];if(names[i])b.textContent=names[i]});parseDesk(specs.value)}};
