...
function canonicalLabels(){const title=$('.top>b,.top>strong');if(title&&/b&l morley/i.test(title.textContent||''))title.textContent='B&L Morley';const navLabels={home:'Home',computer:'Computer',console:'Console',general:'GP'};$$('nav button[data-page]').forEach(button=>{const label=navLabels[button.dataset.page],small=$('small',button);if(label&&small)small.textContent=label})}
