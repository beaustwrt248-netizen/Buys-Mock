...
For a high-risk change, Nova may create the branch, tests, and pull request, but must leave merge pending until Beau explicitly instructs ChatGPT in a user conversation to merge that specific ready pull request after the risk has been presented.

### Single-owner approval rule

Nova currently operates through the same GitHub identity as the repository owner. Therefore a GitHub review, label, comment, reaction, assignment, or other repository action from that identity is not independent human approval for Nova's high-risk work and must never be interpreted as permission to merge.

The Nova PR Guard intentionally keeps protected/high-risk Nova changes blocked. GitHub metadata alone cannot clear that block while Nova and the repository owner share one identity. Explicit conversational authorization from Beau is required before ChatGPT may attempt a high-risk merge, and that authorization applies only to the specific ready PR/head that was presented.
...
- Never treat same-identity GitHub activity as independent approval for high-risk Nova work.
