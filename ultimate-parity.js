(()=>{
'use strict';
const TOKENS={bg:'#f5f7f4',surface:'#ffffff',raised:'#ffffff',accent:'#167a5a',accentStrong:'#0f684c',accentSoft:'#e6f1ec',text:'#1c2b26',secondary:'#52655e',muted:'#73847e',border:'#cedbd5',good:'#167a5a',warn:'#9a6700',bad:'#b42318'};
...
function openHelp(){
  ...
  d.innerHTML=`<div class="morley-ultimate-card"><button class="morley-ultimate-close" aria-label="Close help">×</button><h2>Help & FAQ</h2><p>The same buying, valuation and pricing workflow used across B&L Morley Android and buyshub.me.</p><h3>How to use Morley</h3>${section('1. Choose the right category','Open Categories, then choose Laptops, Desktops, Mobile Phones or Gaming Consoles. Use General Buys / GP for other merchandise.')}${section('2. Identify the item cleanly','Use the clearest model, model code, console variant or hardware specification available so comparable matching stays relevant.')}${section('3. Enter Seller Ask','Seller Ask is the amount requested by the seller. Morley compares it with the protected Max Buy and does not invent it.')}${section('4. Select the correct grade','A targets 30% GP, B 50%, C 70%, and Luxury 30%. Grade changes the buying ceiling.')}${section('5. Review valuation evidence','Exact matches are strongest. Similar/spec matches are supporting evidence. Check confidence and comparable titles before buying.')}${section('6. Save and track the deal','Save useful valuations, add purchased items to Inventory and record completed sales so realised gross profit is tracked.')}${section('7. Android NFC','NFC is an Android-only hardware capability. Morley reports unavailable/disabled hardware, debounces repeated reads and supports common NDEF text/URI records. The website does not pretend to provide phone NFC.')}${section('8. Keep Morley current','Use More → Updates in Android for signed releases. buyshub.me updates from the production web build.')}<h3>Feature breakdown</h3>${section('Categories','Laptops, Desktops, Mobile Phones and Gaming Consoles live together under Categories.')}${section(HELP_GENERAL_LABEL,'A / B / C / Luxury protected buying targets for general merchandise.')}${section('Valuation engine','Model resolution, comparable quality, confidence and target-margin controls. If trustworthy evidence is insufficient, Morley should return unavailable rather than invent a value.')}${section('Quick Deal + Test & Buy','Fast protected buying guidance plus category-specific hardware checks when required.')}${section('Inventory, Scanner & Sales','Track stock, barcode/serial, cost, resale and realised gross profit.')}${section('More tools','Use More → Updates, More → System diagnostics, More → Support and More → Backup & data for maintenance and support workflows.')}<h3>FAQ</h3>${faq('Why are Android and buyshub.me kept the same?','Morley is one product. Shared navigation, colours, wording, pricing workflows and business rules are kept in parity; only platform-specific capabilities differ.')}${faq('Where are Laptops and Desktops?','Open Categories, then choose Laptops or Desktops.')}${faq('Where are Gaming Consoles?','Open Categories → Gaming Consoles for PlayStation, Xbox and Nintendo pricing.')}${faq('Where are Mobile Phones?','Open Categories → Mobile Phones to use the universal phone search and protected pricing workflow.')}${faq('How is Max Buy calculated?','Market or expected sale value × (1 − target GP). A/Luxury use 70% of value, B 50% and C 30%.')}${faq('What do BUY, NEGOTIATE and PASS mean?','BUY is within target. NEGOTIATE means a lower ask may fit. PASS/REJECT means the ask or testing is outside the permitted buying boundary.')}${faq('Why can a valuation be unavailable?','There may not be enough trustworthy exact evidence. That is safer than manufacturing a price from weak matches.')}${faq('Does NFC work on the website?','No. NFC is kept as a real Android hardware feature rather than being falsely emulated in the web client.')}${faq('What if something looks wrong?','Check identity, grade, Seller Ask, evidence and confidence, then use More → System diagnostics or More → Support with the model/search term and a screenshot where possible.')}
</div>`;
...
let lastHealth='';
function integrity(){
  const required=['categories','general buys','more'];
  const scope=(document.body?.textContent||'').toLowerCase();
