...
class ConsolePricingCatalogTest {
    ...
    fun suppliedPricingSheetIsFullyRepresented() {
        ...
        assertEquals(149.0, ConsolePricingCatalog.entries.first { it.name == "Sony PS4 OG 500gb" }.rrp, 0.01)
        assertEquals(189.0, ConsolePricingCatalog.entries.first { it.name == "Sony PS4 OG 1tb" }.rrp, 0.01)
        assertEquals(229.0, ConsolePricingCatalog.entries.first { it.name == "Sony PS4 Slim 500gb" }.rrp, 0.01)
        assertEquals(269.0, ConsolePricingCatalog.entries.first { it.name == "Sony PS4 Slim 1tb" }.rrp, 0.01)
