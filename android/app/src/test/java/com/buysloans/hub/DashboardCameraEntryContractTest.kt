...
class DashboardCameraEntryContractTest {
    ...
    fun dashboardKeepsScanDeviceButDoesNotDuplicateGoogleCameraSearch() {
        ...
        val paletteMigration = File("../apply_morley_brand_palette.py").readText()
        ...
        assertFalse(paletteMigration.contains("duplicate_start ="))
        assertFalse(paletteMigration.contains("Google camera search now use the same Google Play services camera scanner"))
    }
}
