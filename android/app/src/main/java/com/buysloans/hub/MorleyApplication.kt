...
class MorleyApplication : Application(), Application.ActivityLifecycleCallbacks {
    ...
    override fun onCreate(){super.onCreate();instance=this;registerActivityLifecycleCallbacks(this);AppCopyrightFooter.install(this);NotificationHelper.createChannels(this);UpdateCheckScheduler.schedule(this);ReleasePolicyCoordinator.register(this);LiveDevicePricing.cached(this);if(AuthManager.isSignedIn(this)){ensureCatalogueLiveSync();appScope.launch{runCatching{LiveDevicePricing.reconcile(this@MorleyApplication)}};FirebaseMessaging.getInstance().token.addOnSuccessListener{token->DeviceRegistrar.register(this,token)}}}
    private fun ensureCatalogueLiveSync(){if(!AuthManager.isSignedIn(this)){catalogueSyncJob?.cancel();catalogueSyncJob=null;return};if(catalogueSyncJob?.isActive==true)return;catalogueSyncJob=appScope.launch{while(isActive){runCatching{LiveDevicePricing.reconcile(this@MorleyApplication)};delay(1_000L)}}}
    ...
    override fun onActivityResumed(activity:Activity){DiagnosticContextStore.recordActivity(this,activity::class.java.simpleName);ensureCatalogueLiveSync();appScope.launch{runCatching{LiveDevicePricing.reconcile(this@MorleyApplication)}};checkMaintenance(activity)}
