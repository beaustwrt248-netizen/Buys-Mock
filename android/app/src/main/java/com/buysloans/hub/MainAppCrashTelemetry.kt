...
object MainAppCrashTelemetry {
    ...
    suspend fun flushPending(context: Context): Boolean {
        ...
        val payload = JSONObject().apply {
            put("app_version", sanitize(BuildConfig.VERSION_NAME))
            put("device_model", sanitize("${Build.MANUFACTURER} ${Build.MODEL}".trim()).ifBlank { "Unknown" })
            put("failing_screen", crash.failingScreen)
            put("error_class", crash.errorClass)
            put("occurred_at", Instant.ofEpochMilli(crash.occurredAt).toString())
        }
        val code = runCatching {
            val connection = (URL("${BuildConfig.SUPABASE_URL}/rest/v1/admin_error_events").openConnection() as HttpURLConnection).apply {
                requestMethod = "POST"
                connectTimeout = 5000
                readTimeout = 5000
                doOutput = true
