...
internal object OtaFeaturePolicy {
    internal fun enabledFromConfig(rows: JSONArray): Boolean {
        for (i in 0 until rows.length()) {
            ...
            val flags = row.optJSONObject("value") ?: return otaEnabledValue(null)
            return otaEnabledValue(if (flags.has("otaEnabled")) flags.optBoolean("otaEnabled", true) else null)
        }
        return otaEnabledValue(null)
    }
