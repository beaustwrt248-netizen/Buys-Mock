...
object ComparableSalesQuality {
    fun assess(comparables: List<ComparableSaleEvidence>): ComparableSalesAssessment {
        ...
        if (usable.isEmpty()) {
            return ComparableSalesAssessment(0.0, 0, 0, 0.0, 0.0, true, listOf("No usable sold comparables"))
        }
        val sortedPrices = usable.map { it.salePrice }.sorted()
        val median = if (sortedPrices.size % 2 == 1) sortedPrices[sortedPrices.size / 2] else {
            val upper = sortedPrices.size / 2
            (sortedPrices[upper - 1] + sortedPrices[upper]) / 2.0
        }
        val spread = if (median > 0.0) (usable.maxOf { it.salePrice } - usable.minOf { it.salePrice }) / median else 0.0
        val evidenceScores = usable.map { comparable ->
            var score = 0.0
            if (comparable.verifiedSold) score += 0.25
            if (comparable.exactModelMatch) score += 0.25 else if (comparable.sameGeneration) score += 0.12
            if (comparable.sameConditionBand) score += 0.15
            if (comparable.trustedSource) score += 0.15
            score += when { comparable.ageDays <= 14 -> 0.20; comparable.ageDays <= 30 -> 0.16; comparable.ageDays <= 60 -> 0.10; comparable.ageDays <= 90 -> 0.05; else -> 0.0 }
            score.coerceIn(0.0, 1.0)
        }
        var quality = evidenceScores.average() * (usable.size / 5.0).coerceIn(0.35, 1.0)
        ...
        return ComparableSalesAssessment(quality, usable.size, exactCount, median, abs(spread) * 100.0, stale, reasons)
    }
}
