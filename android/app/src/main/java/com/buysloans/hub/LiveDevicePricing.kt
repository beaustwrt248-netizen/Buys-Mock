...
object LiveDevicePricing {
    ...
    fun find(prices: List<LiveDevicePrice>, deviceCatalogId: Long, storage: String): LiveDevicePrice? = prices.firstOrNull {
        it.deviceCatalogId == deviceCatalogId && it.authoritative && normalizeStorage(it.storage) == normalizeStorage(storage)
    }
    ...
    suspend fun refresh(context: Context): List<LiveDevicePrice> = withContext(Dispatchers.IO) {
        ...
        val connection = (URL("${BuildConfig.SUPABASE_URL}/functions/v1/admin-pricing-control").openConnection() as HttpURLConnection).apply {
        ...
        try {
            connection.outputStream.use { it.write(JSONObject().put("action", "public_list").toString().toByteArray()) }
            ...
            val raw = body.optJSONArray("prices")?.toString() ?: "[]"
    ...
    private fun parse(raw: String): List<LiveDevicePrice> = runCatching {
        ...
        buildList {
            for (index in 0 until array.length()) {
                ...
                val id = row.optLong("device_catalog_id", -1L); val storage = row.optString("storage").trim(); val price = row.optDouble("price_aud", Double.NaN)
