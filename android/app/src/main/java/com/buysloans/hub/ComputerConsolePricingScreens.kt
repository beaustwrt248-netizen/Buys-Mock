            ...
            Card(
                ...
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    Button(
                        ...
                        modifier = Modifier.fillMaxWidth(),
                        colors = ButtonDefaults.buttonColors(containerColor = MorleyAccentStrong, contentColor = MorleyTextPrimary),
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Card(
            ...
            Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Text("CONSOLE PRICING", fontSize = 11.sp, fontWeight = FontWeight.Black, color = MorleyAccent)
                Text("Select console + condition grade", fontSize = 22.sp, fontWeight = FontWeight.Black, color = MorleyTextPrimary)
                Text(
                    "Buy price automatically follows Morley's standard grade rules: A 70% • B 50% • C 30% of the supplied console price-sheet value.",
                    color = MorleyTextSecondary,
                    fontSize = 13.sp
                )
            }
        ...
        Box(Modifier.fillMaxWidth()) {
            OutlinedButton(
                ...
                border = BorderStroke(1.dp, MorleyBorder),
                shape = RoundedCornerShape(16.dp)
            ) { Text(selected?.name ?: "Choose console", color = MorleyTextPrimary) }
            DropdownMenu(expanded = menuOpen, onDismissRequest = { menuOpen = false }) {
                ConsolePricingCatalog.entries.forEach { entry ->
                    DropdownMenuItem(
                        text = { Text("${entry.name} — ${money.format(entry.rrp)}") },
                        onClick = { selected = entry; menuOpen = false }
                    )
                }
            }
        }
        selected?.let { console ->
            ...
            Card(
                ...
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    Text(money.format(console.rrp), fontSize = 28.sp, fontWeight = FontWeight.Black, color = MorleyTextPrimary)
                    ...
                    Text("${money.format(console.rrp)} × $percentage% = ${money.format(buyPrice)}", color = MorleyTextSecondary, fontSize = 12.sp)
                }
            }
        }
        Text("Supported consoles", color = MorleyTextPrimary, fontWeight = FontWeight.Black, fontSize = 16.sp)
        ConsolePricingCatalog.entries.forEach { entry ->
            Card(
                ...
                Row(Modifier.fillMaxWidth().padding(14.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                    ...
                    Text(money.format(entry.rrp), color = MorleyAccent, fontWeight = FontWeight.Black)
                }
            }
        }
    }
