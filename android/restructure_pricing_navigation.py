...
'MobilePhonePricingScreen.kt':('featuredPhoneBrands = listOf("Apple", "Samsung", "Google", "OnePlus", "Xiaomi")','Search all mobile phones','MobilePhoneDeviceCatalog.search(globalQuery)','MobilePhoneDeviceCatalog.models(brand)','MobilePhoneDeviceCatalog.pricedEntry(it)','MobilePhonePricingCatalog.gradeBuyPercent','MobilePhonePhoto('),
'MobilePhoneDeviceCatalog.kt':('ExpandedMobilePhoneCatalog.models','fun search(query:String,limit:Int=80)','LiveDevicePricing.find(entry.brand,entry.model,entry.modelNumber,entry.storage)','MobilePhonePricingCatalog.entries.firstOrNull')}
...
forbidden={'DashboardActivity.kt':('HISTORY("History", MorleyIcons.History)','NavCard(MorleyIcons.Categories, "Categories", "Laptops, desktops, mobile phones and gaming consoles", onCategories)','NavCard(MorleyIcons.Console, "Console Pricing", "PS4, PS5, Xbox and Nintendo grade pricing", onConsole)')}
missing=[]
...
if present: raise SystemExit('Removed navigation shortcuts returned: '+' | '.join(present))
print('Adaptive dashboard, Categories, GP navigation, global mobile search, imagery, and live-authoritative priced/unpriced phone flows aligned')
