...
context.window.morleyDesktopGo('mobilePhones');
assert.equal(shown.at(-1), 'mobilePhones', 'Mobile Phones must open from Categories');
context.window.morleyDesktopGo('categories');
assert.equal(shown.at(-1), 'categories', 'Back to Categories must work');
context.window.morleyDesktopGo('invalid-route');
assert.equal(shown.length,3,'unknown routes remain rejected');
