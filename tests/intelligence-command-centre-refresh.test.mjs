...
test('Command Centre refreshes all available non-marketplace live evidence adapters',async()=>{
  const code=await src();
  for(const pattern of [/MorleyDeviceIntelligenceAdapter\?\.refresh/,/MorleyPricingIntelligenceAdapter\?\.refresh/,/refreshMorleySupportIntelligenceEvidence/,/refreshMorleyReleaseIntelligenceEvidence/]) assert.match(code,pattern);
  assert.doesNotMatch(code,/MorleyMarketplaceLiveSource\?\.refresh/);
