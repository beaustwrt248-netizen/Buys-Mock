...
async function newTarget(port){const target=await jsonRetry(`http://127.0.0.1:${port}/json/new?about:blank`,{method:'PUT'});const cdp=new Cdp(target.webSocketDebuggerUrl);await cdp.open();await cdp.call('Page.enable');await cdp.call('Runtime.enable');await cdp.call('Network.enable');await cdp.call('Network.setBlockedURLs',{urls:['*.js','*.mjs']});return cdp}
...
function startChrome(chrome){const profile=fs.mkdtempSync(path.join(os.tmpdir(),'morley-browser-smoke-'));const proc=cp.spawn(chrome,['--remote-debugging-port=0','--remote-debugging-address=127.0.0.1','--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--allow-file-access-from-files','--no-first-run','--no-default-browser-check',`--user-data-dir=${profile}`,'about:blank'],{stdio:['ignore','ignore','pipe']});let stderr='';proc.stderr.on('data',d=>{stderr+=String(d)});return {profile,proc,getStderr:()=>stderr}}
async function launchChrome(chrome){let lastError=null;for(let attempt=1;attempt<=CHROME_START_ATTEMPTS;attempt++){const started=startChrome(chrome);try{const port=await waitForDevTools(started.proc,started.getStderr);await jsonRetry(`http://127.0.0.1:${port}/json/version`,{},6,300);return {...started,port}}catch(error){lastError=error;await stopChrome(started.proc);try{fs.rmSync(started.profile,{recursive:true,force:true})}catch{}if(attempt<CHROME_START_ATTEMPTS)await sleep(750)}}throw new Error(`${lastError?.message||'Chrome failed to start'} after ${CHROME_START_ATTEMPTS} attempts`)}
async function run(){
