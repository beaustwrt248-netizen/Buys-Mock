...
The live catalogue is realtime-first across Web, Android, Admin and Nova. That existing realtime implementation is the canonical propagation path for catalogue mutations; ecosystem consolidation must extend that pattern rather than introduce another sync layer.
...
7. Existing realtime catalogue sync is reused as Morley Core infrastructure rather than duplicated by a second ecosystem sync implementation.
