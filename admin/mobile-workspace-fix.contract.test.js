...
for(const token of ['enforceSingleWorkspace','panel.id!==id','admin-mobile-user-toggle','enhanceReleaseIntegrity','moveToWorkspaceTop','observerScheduled'])if(!js.includes(token))throw new Error('missing Admin mobile workspace behavior: '+token);
if(js.includes('new MutationObserver(()=>requestAnimationFrame(run))'))throw new Error('mobile workspace mutation refreshes must be coalesced to one animation frame');
if(!home.includes('mobile-workspace-fix.css?v=2')||!home.includes('mobile-workspace-fix.js?v=3'))throw new Error('final mobile workspace authority is not loaded with the current cache version');
