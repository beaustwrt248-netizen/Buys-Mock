...
for(const token of [
  '#tab-overview.hidden{display:none!important}',
  '#adminHomeQuick{display:none!important}',
  '#usersList>.row .actions{display:none!important}',
  '#usersList>.row.mobile-expanded .actions{display:flex!important}',
  '#tab-release .grid{grid-template-columns:repeat(2,minmax(0,1fr))!important',
  'padding-bottom:calc(92px + env(safe-area-inset-bottom))!important',
  '.tabs{position:fixed!important'
])if(!css.includes(token))throw new Error('missing Admin mobile workspace rule: '+token);
