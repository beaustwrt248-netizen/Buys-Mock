...
const state={catalogue:[],pricing:[],marketplace:[],system:[],images:[],updatedAt:null,sources:{}};const listeners=new Set();
...
function snapshot(){return Object.freeze({updatedAt:state.updatedAt,sources:clone(state.sources),catalogue:catalogue(),pricing:pricing(),pricingObservations:clone(state.pricing),systemEvidence:clone(state.system),imageEvidence:clone(state.images),marketplace:marketplace(),alerts:alerts()})}
function subscribe(fn){if(typeof fn!=='function')return()=>{};listeners.add(fn);return()=>listeners.delete(fn)}
function clear(kind){if(kind&&Object.prototype.hasOwnProperty.call(state,kind)&&!['sources','updatedAt'].includes(kind)){state[kind]=[];delete state.sources[kind];return publish()}for(const k of ['catalogue','pricing','marketplace','system','images'])state[k]=[];state.sources={};return publish()}
