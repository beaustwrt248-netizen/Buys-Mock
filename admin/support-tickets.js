...
async function saveTicket(){if(!currentTicket)return;ensureTicketControls();const ticketId=currentTicket.id,isCurrent=()=>currentTicket?.id===ticketId,updates={status:q('ticketStatus').value,priority:q('ticketPriority').value,assigned_to:q('ticketAssignee').value||null};const {error}=await sb.from('support_tickets').update(updates).eq('id',ticketId);if(isCurrent())q('ticketAdminStatus').textContent=error?'Ticket update failed.':'Ticket updated.';if(!error){if(isCurrent())await openTicket(ticketId);await loadTickets();if(window.loadAudit)await loadAudit()}}
...
async function reply(){if(!currentTicket)return;const ticket=currentTicket,ticketId=ticket.id,isCurrent=()=>currentTicket?.id===ticketId,body=q('ticketReply').value.trim();if(!body)return;const {data:{user}}=await sb.auth.getUser();const actor=user?.id;if(!actor){if(isCurrent())q('ticketAdminStatus').textContent='Support session identity is unavailable.';return}const {data:message,error}=await sb.from('support_ticket_messages').insert({ticket_id:ticketId,author_user_id:actor,author_role:'admin',body}).select('id').single();if(isCurrent())q('ticketAdminStatus').textContent=error?'Reply could not be sent.':'Reply added. Emailing user…';if(!error){if(isCurrent())q('ticketReply').value='';if(ticket.status==='open')await sb.from('support_tickets').update({status:'in_progress'}).eq('id',ticketId);const {data:mail,error:mailError}=await sb.functions.invoke('send-morley-email',{body:{action:'support_ticket_reply',ticket_id:ticketId,message_id:message.id}});if(isCurrent())q('ticketAdminStatus').textContent=mailError?`Reply saved, but email was not sent: ${await emailErrorMessage(mailError)}`:mail?.ok?'Reply sent and user emailed.':'Reply saved, but email delivery was not confirmed.';if(isCurrent())await openTicket(ticketId);await loadTickets();}}
