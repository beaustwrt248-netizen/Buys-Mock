(function(){
  ...
  function removeRenderedChallenge(){
  ...
  document.addEventListener('visibilitychange',function(){
    if(document.visibilityState==='visible'&&!captchaToken&&widgetId===null)resetChallenge('Security check loading…',!window.turnstile);
  });
  window.addEventListener('pageshow',function(event){
    if(event.persisted&&!captchaToken)resetChallenge('Security check loading…',!window.turnstile);
  });

  syncLoginEnabled();
  resetChallenge('Security check loading…',false);
  ...
  loginBtn.onclick=async function(){
    ...
    try{
      ...
      if(error){loginStatus.textContent=error.message;resetChallenge('Complete a new security check to retry.',false);return;}
      await loadSession();
    }catch(error){
      loginStatus.textContent=error&&error.message?error.message:'Sign in failed. Please retry.';
      resetChallenge('Complete a new security check to retry.',false);
    }finally{
      busy=false;
      syncLoginEnabled();
    }
