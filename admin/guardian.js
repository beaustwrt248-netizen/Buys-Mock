(()=>{
  ...
  const states=['queued','diagnosing','proposed','awaiting_approval','applying','verifying','resolved','failed','ignored'];
  ...
  async function loadGuardian(){
    ...
    const open=rows.filter(x=>!['resolved','ignored'].includes(x.state)).length;
    const approvals=rows.filter(x=>x.state==='awaiting_approval').length;
    const resolved=rows.filter(x=>x.state==='resolved').length;
    const failed=rows.filter(x=>x.state==='failed').length;
    $('guardianMetricOpen').textContent=open;
    $('guardianMetricApproval').textContent=approvals;
    $('guardianMetricResolved').textContent=resolved;
    $('guardianMetricFailed').textContent=failed;
    $('guardianStatus').textContent=s.enabled
      ? `ONLINE • Report-a-Problem intake active • ${s.auto_fix_enabled?'low-risk auto-fix permission enabled':'auto-fix permission locked'}`
  ...
  async function saveGuardian(){
    ...
    const payload={
      enabled:$('guardianEnabled').checked,
      auto_fix_enabled:$('guardianAutoFix').checked,
      max_auto_risk:$('guardianMaxRisk').value,
      require_human_for_code:$('guardianHumanCode').checked,
      updated_by:user?.id||null
    };
    if(!payload.require_human_for_code){
      $('guardianSaveStatus').textContent='Code-changing fixes must keep human approval enabled in Guardian Core.';
  ...
  document.querySelector('[data-tab="guardian"]')?.addEventListener('click',loadGuardian);
